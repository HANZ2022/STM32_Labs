...
static void PrintTaskId(void *params)
{
	...
	int id = *taskId;
	char ibuff[50];
	TickType_t startTime = xTaskGetTickCount(); // Get the starting tick time
	while (1)
	{
		TickType_t currentTime = xTaskGetTickCount(); // Get the current tick time
		TickType_t ExistTime = currentTime - startTime;
        if (ExistTime >= ((1000/portTICK_RATE_MS)*20))
        {
        //If the existing time exceed the range, delete the task
			sprintf(ibuff,"Deleted task new task %d \r\n", id);
			print_str(ibuff);
			*taskId = *taskId - 1; //Decrease the task
        	vTaskDelete(NULL); //Delete the Sub_task
        }

        sprintf(ibuff,"Task %d is printing\r\n", id);
        print_str(ibuff);
...
static void main_task(void *params)
{
	...
	do {
		...
		val = xPortGetFreeHeapSize() ; //Get the free heap size
