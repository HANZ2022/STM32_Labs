...
void send_sensorData_message(enum SensorId_t sensorType, uint16_t data){
	...
	switch(sensorType){
	case PH:
		sprintf(tx_sensor_buffer, "$PHVAL,03,%08u,*,00\n", data);
		break;
	case Depth:
		sprintf(tx_sensor_buffer, "$DEPTH,03,%08u,*,00\n", data);
		break;
	case Temperature:
		sprintf(tx_sensor_buffer, "$TEMPE,03,%08u,*,00\n", data);
		break;
	case Oxygen:
		sprintf(tx_sensor_buffer, "$OXYGE,03,%08u,*,00\n", data);
		break;
	default:
		break;
	}
...
void send_sensorEnable_message(enum SensorId_t sensorType, uint16_t TimePeriod_ms){
	...
	switch(sensorType){
	case PH:
		sprintf(tx_sensor_buffer, "$PHVAL,00,%08u,*,00\n", TimePeriod_ms);
		break;
	case Depth:
		sprintf(tx_sensor_buffer, "$DEPTH,00,%08u,*,00\n", TimePeriod_ms);
		break;
	case Temperature:
		sprintf(tx_sensor_buffer, "$TEMPE,00,%08u,*,00\n", TimePeriod_ms);
		break;
	case Oxygen:
		sprintf(tx_sensor_buffer, "$OXYGE,00,%08u,*,00\n", TimePeriod_ms);
		break;
	default:
		break;
	}
